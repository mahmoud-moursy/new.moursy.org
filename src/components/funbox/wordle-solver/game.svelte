<script lang="ts">
  ...
  import { bounceOnEvent, shakeOnEvent, shakeUp, withTooltip } from "../interactions.svelte";
  ...
  let ollKorrect = $derived(guesses[guesses.length - 1]?.every((i) => i.status === "correct"));
  let submitButton;

  let inputElements: (HTMLInputElement | undefined)[] = $state([undefined, undefined, undefined, undefined, undefined]);
  ...
  function tryGuessing(e: Event) {
    ...
    inputs = inputs.map((inp, idx) => ({
      value: ollKorrect ? green[idx] : wordRanking.length > 0 ? wordRanking[0][0][idx] : "",
  ...
  const statusEffects = {
    correct: "bg-emerald-700 border-emerald-700! text-white aspect-square",
    present: "bg-yellow-700 border-yellow-700! text-white",
    absent: "bg-slate-700 border-slate-700! text-white",
  };
  const tooltipInput = `Input your word into the boxes (<p class="${statusEffects["correct"]} w-5 h-5 items-center justify-center inline-flex">✓</p>, <p class="${statusEffects["present"]} w-5 h-5 items-center justify-center inline-flex">?</p>, <p class="${statusEffects["absent"]} w-5 h-5 items-center justify-center inline-flex">✗</p>)`;
  const tooltipShortcuts = `Use shortcuts 1 (Green), 2 (Yellow), 3 (Gray) to type words faster.`;
</script>

<form class=" gap-4 p-12 max-w-sm mx-auto flex flex-col items-center" onsubmit={tryGuessing}>
  <section>
    <section class="grid grid-cols-5 z-10 gap-4 w-sm" {@attach withTooltip("The 'best guess' will appear here!")}>
      ...
      {#if !ollKorrect}
        ...
        {:else}
          ...
          <div
            ...
            <p>
              {#if filterError}
                The filter combination you put in was impossible, so you should probably reset everything and try
                putting it in again... 🤤
              {:else}
                Seems you've exhausted the word list... either The Wordle Solver is broken, or <em>you</em>
                are.
              {/if}
            </p>
          </div>
  ...
  <section class="flex flex-col mx-auto">
    <section class="grid grid-cols-5 *:col-span-1 gap-1" {@attach withTooltip(tooltipInput)}>
      {#each inputs as input, idx (idx)}
        <FillBox
          {@attach idx === 0 && withTooltip(tooltipShortcuts)}
    ...
    <button
      ...
      {@attach withTooltip("Tap me to lock-in your word input.")}
